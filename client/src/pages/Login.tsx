...
import { runningInApp } from "@/lib/native";
...
export default function Login() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (json?.ok) {
        // App Store reviewer shortcut — server issues session directly,
        // no email round-trip. Drop them into the app.
        if (json?.reviewer) {
        ...
        <div className="w-full max-w-[420px]">
          ...
          {phase === "enter" && (
            ...
            >
              ...
              {/* Primary: Google OAuth — web only. In the iOS app we hide
                  this until the deep-link callback flow ships (post-launch),
                  because Safari View Controller cookies don't cross back into
                  the WKWebView. */}
              {!runningInApp() && (
                <>
                  <a
